...
class SideMenu {
	...
	render () {
		...
		const $btnSetDim = $(`<button class="btn btn-primary" style="width: 100%;">設置分層</div>`).appendTo($wrpSetDim);
		$btnSetDim.on("click", () => {
		...
		const $btnLockPanels = $(`<button class="btn btn-danger" title="鎖定皮膚"><span class="glyphicon glyphicon-lock"/></button>`).appendTo($wrpFullscreen);
		...
		const $btnLoadFile = $(`<button class="btn btn-primary">讀取文件</button>`).appendTo($wrpSaveLoadFile);
		...
		const $btnReset = $(`<button class="btn btn-danger" style="width: 100%;">重設螢幕</button>`).appendTo($wrpReset);
		$btnReset.on("click", () => {
...
class AddMenuVideoTab extends AddMenuTab {
	...
	render () {
		if (!this.$tab) {
			...
			const $iptUrlYT = $(`<input class="form-control" placeholder="黏貼YouTube URL">`)
				.on("keydown", (e) => {
					if (e.which === 13) $btnAddYT.click();
				})
				.appendTo($wrpYT);
			const $btnAddYT = $(`<button class="btn btn-primary btn-sm">嵌入</button>`).appendTo($wrpYT);
			$btnAddYT.on("click", () => {
			...
			const $iptUrlTwitch = $(`<input class="form-control" placeholder="黏貼Twitch URL">`)
				.on("keydown", (e) => {
					if (e.which === 13) $btnAddTwitch.click();
				})
				.appendTo($wrpTwitch);
			const $btnAddTwitch = $(`<button class="btn btn-primary btn-sm">嵌入</button>`).appendTo($wrpTwitch);
			const $btnAddTwitchChat = $(`<button class="btn btn-primary btn-sm">嵌入聊天室</button>`).appendTo($wrpTwitch);
			...
			const $iptUrlGeneric = $(`<input class="form-control" placeholder="黏貼任何URL">`)
				.on("keydown", (e) => {
					if (e.which === 13) $iptUrlGeneric.click();
				})
				.appendTo($wrpGeneric);
			const $btnAddGeneric = $(`<button class="btn btn-primary btn-sm">嵌入</button>`).appendTo($wrpGeneric);
...
class AddMenuImageTab extends AddMenuTab {
	...
	render () {
		if (!this.$tab) {
			...
			const $iptUrl = $(`<input class="form-control" placeholder="黏貼圖片 URL">`)
				.on("keydown", (e) => {
...
class AddMenuSpecialTab extends AddMenuTab {
	...
	render () {
		if (!this.$tab) {
			...
			$$`<div class="ui-modal__row">
			<span>先攻追蹤器：玩家查看頁</span>
			${$btnPlayertracker}
			</div>`.appendTo($tab);

			$(`<hr class="ui-modal__row-sep"/>`).appendTo($tab);

			const $wrpText = $(`<div class="ui-modal__row"><span>基礎文本塊 <i class="text-muted">（如需功能齊全編輯器，可嵌入 Google Doc 或類似編輯器）</i></span></div>`).appendTo($tab);
...
class UnitConverter {
	static make$Converter (board, state) {
		const units = [
			new UnitConverterUnit("寸", "2.54", "公分", "0.394"),
			new UnitConverterUnit("尺", "0.305", "米", "3.28"),
			new UnitConverterUnit("裏", "1.61", "公里", "0.620"),
